...
class CarSpider(FinnBaseSpider):
    ...
    async def parse(self, response):
        ...
        item['email'] = None #company_profile.get('contacts', [{}])[0].get('email') if company_profile.get('contacts') else company_profile.get('contact', {}).get('email')
        item['web'] = None #company_profile.get('homepageUrl', None)
